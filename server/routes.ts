  ...
  app.patch("/api/v2/photos/select", authenticate, async (req: Request, res: Response) => {
    try {
      ...
      if (isUUID(projectId)) {
        ...
        // Auto-save: salva o estado completo de seleção enviado pelo cliente.
        // Guard: se photoIds vier vazio, ignora para evitar apagar seleções por race condition.
        // O race condition de inicialização é prevenido pelo isProjectLoaded no cliente.
