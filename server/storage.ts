...
export class DatabaseStorage implements IStorage {
  ...
  async getPortfolio(id: number): Promise<any | undefined> {
    try {
      ...
      if (!portfolio) {
        console.log(`DatabaseStorage: Portfólio ID=${id} não encontrado`);
        return undefined;
      }

      // Get photos for the portfolio
      const photos = await db
        .select()
        .from(portfolioPhotos)
      ...
      console.log(`DatabaseStorage: Portfólio ID=${id} encontrado com ${photos.length} fotos`);
      return {
        ...portfolio,
        photos: photos
      };
  ...
  async getPortfolioBySlug(slug: string): Promise<any | undefined> {
    try {
      ...
      if (!portfolio) {
        console.log(`DatabaseStorage: Portfólio com slug='${slug}' não encontrado`);
        return undefined;
      }
      ...
      console.log(`DatabaseStorage: Portfólio público slug='${slug}' encontrado com ${photos.length} fotos`);
      return {
        ...portfolio,
        photos: photos
      };
  ...
  async getPublicPortfolio(slug: string): Promise<any | undefined> {
    try {
      ...
      if (!portfolio) {
        console.log(`DatabaseStorage: Portfólio público com slug='${slug}' não encontrado`);
        return undefined;
      }
  ...
  async updatePortfolio(id: number, data: any): Promise<any | undefined> {
    try {
      ...
      if (!updatedPortfolio) {
        console.log(`DatabaseStorage: Portfólio ID=${id} não encontrado para atualização`);
        return undefined;
      }
  ...
  async deletePortfolio(id: number): Promise<boolean> {
    try {
      // First delete all photos in the portfolio
      await db
  ...
  async addPhotosToPortfolio(portfolioId: number, photoUrls: string[]): Promise<any[]> {
    try {
      // Get current highest order in portfolio
  ...
  async getUserProjects(userId: number): Promise<any[]> {
    try {
      ...
      // Format projects for portfolio photo selection
      ...
      console.log(`DatabaseStorage: Encontrados ${formattedProjects.length} projetos para usuário ID=${userId}`);
  ...
  async getPortfolioPhotos(portfolioId: number): Promise<any[]> {
    try {
      ...
      console.log(`DatabaseStorage: Encontradas ${photos.length} fotos no portfólio ID=${portfolioId}`);
      return photos;
    } catch (error) {
